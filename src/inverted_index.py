...
class inverted_index:
    ...
    def build(self, documents):
        ...
        for file_name, file_content in documents.items():
            ...
            progress_bar.print_progress_bar(compteur, taille, prefix = 'Indexation des fichiers : ' + str(compteur) +  '/' + str(taille), suffix = '')
            compteur = compteur + 1
    ...
    def search(self, request1):
        ...
        #On appelle la fonction de la partie jaune qui recherche les mots similaires
    ...
    def sort(self, documents_cleaner):
        ...
        for file,content in documents_cleaner.items():
            ...
            #Progress bar         
            progress_bar.print_progress_bar(compteur, taille, prefix = 'Récupération des fichiers : ' + str(compteur) +  '/' + str(taille), suffix = '')
            compteur = compteur + 1
        compteur = 1
        ...
        for word,files in self.index.items():
            dict_word = dict()
            for file in files:
                score_word_file = tf_idf.TF_frequence_brute(word,documents_cleaner[file])
            ...
            #Progress bar
            progress_bar.print_progress_bar(compteur, taille, prefix = 'Tri des fichiers (par score de mot) : ' + str(compteur) +  '/' + str(taille), suffix = '')
            compteur = compteur + 1
